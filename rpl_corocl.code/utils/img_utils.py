...
def crop_center(img, gt, cropx, cropy):
    y, x = gt.shape
    startx = x//2-(cropx//2)
    starty = y//2-(cropy//2)
    return img[starty:starty+cropy, startx:startx+cropx, :], gt[starty:starty+cropy, startx:startx+cropx]
...
def pad_image_to_shape(img, shape, border_mode=cv2.BORDER_CONSTANT, value=0):
...
def random_mirror(img, gt, prob=.2):
...
def center_crop_to_shape(img, gt, size):
    if img.shape[0] < size[0]:
        img, _ = pad_image_to_shape(img, (size[0], img.shape[1]), cv2.BORDER_CONSTANT, 0)
        gt, _ = pad_image_to_shape(gt, (size[0], img.shape[1]), cv2.BORDER_CONSTANT, 255)
    if img.shape[1] < size[1]:
        img, _ = pad_image_to_shape(img, (img.shape[0], size[1]), cv2.BORDER_CONSTANT, 0)
        gt, _ = pad_image_to_shape(gt, (img.shape[0], size[1]), cv2.BORDER_CONSTANT, 255)

    if img.shape[0] >= size[0] and img.shape[1] >= size[1]:
        img, gt = crop_center(img, gt, size[0], size[1])
